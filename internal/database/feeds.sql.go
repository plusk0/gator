...
const markFetchedFeeds = `-- name: MarkFetchedFeeds :one
UPDATE feeds
...
func (q *Queries) MarkFetchedFeeds(ctx context.Context, id uuid.UUID) (Feed, error) {
	row := q.db.QueryRowContext(ctx, markFetchedFeeds, id)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
